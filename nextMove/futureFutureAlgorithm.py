...
def goodProspects(currentNode):
    ...
    while (sgDq):
        ...
        values = nextSg.getOrderedValues();

        # check if stale move
        if (np.array_equal(currentSg.intGrid, nextSg.intGrid)):
            continue;
        # check if top tile is moved out of place
        elif (values[0] != nextSg.intGrid[0][0] and values[0] > 4):
            continue;
...
def pickyFilter(currentNode):
    ...
    # check if top tile is moved out of place
        ...
        return False;
    elif (not goodProspects(currentNode)):
        return False;
